...
export const submitBookingAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    console.log("Processing booking request on server:", data);
    ...
    // 3. Dispatch Emails if nodemailer is configured
    if (!emailUser || !emailPass) {
      console.warn("Email credentials missing (EMAIL_USER/EMAIL_PASS). Skipping email dispatch.");
      return { 
        success: true, 
        message: "Booking requested successfully! (Note: Email configuration is missing on server)" 
      };
    }
